...
module.exports.addOrRemoveStarredRoommateProfile = async (req, res) => {
    jwt.verify(req.token, process.env.JWT_SECRET, async (err, authData) => {
      if (err) {
        res.send(
          "error while verifying token in addOrRemoveStarredRoommateProfile"
        );
      } else {
        const { roommateId } = req.body;
        const existingRecord = await StarredRoommateProfiles.findOne({
...
module.exports.userStarredRoommateProfiles = async (req, res) => {
    jwt.verify(req.token, process.env.JWT_SECRET, async (err, authData) => {
      const userStarredRoommateProfiles = await StarredRoommateProfiles.findOne(
        { user: authData.user._id }
      );
